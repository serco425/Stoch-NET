...
b = ones(1,no_of_hidden_layers+1);
    ...
    %1 HL Weight Bit-Flips
    counter_HL = 0;
    for i = 1:392000
        if w1_double(randi([1,size(w1_double,1)],1),randi([1,size(w1_double,2)],1)) == 1 && counter_HL <= 39200
            w1_double(randi([1,size(w1_double,1)],1),randi([1,size(w1_double,2)],1)) = -1;
...
for j = 1:number_of_possible_in_TEST %possible input trials
    ...
    for i = 1:(no_of_hidden_layers+1) % +2 is for input and the output layer
        ...
        if i==1
            layer_wise_test_DET{1} = double(test__data(j,:))*w1_double + double(b(1))*double(cell2mat(b_w(1,1,1)));
        ...
        if i==2
            layer_wise_test_DET{2} = layer_wise_test_DET_ACT{1}*w2_double + double(b(2))*double(cell2mat(b_w(1,2,1)));
            %************************SIGMOID -> SIGN***********************
        ...
        if i==3
            layer_wise_test_DET{3} = layer_wise_test_DET_ACT{2}*w3_double + double(b(3))*double(cell2mat(b_w(1,3,1)));
            %***************************SOFTMAX***************************
